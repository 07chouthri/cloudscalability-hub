...
const IndustriesSection = () => {
  ...
  return (
    <section ref={sectionRef} className="py-20 bg-white" aria-labelledby="industries-heading">
      <div className="container max-w-7xl mx-auto px-4">
        ...
        {/* Industry Cards */}
        <div className="grid grid-cols-2 md:grid-cols-3 gap-8" role="list" aria-labelledby="industries-heading">
          {industries.map(({ icon, name, description }, index) => (
            <div
              key={index}
              role="listitem"
              className={cn(
                "bg-gray-50 rounded-xl p-6 text-center shadow-sm border border-gray-100 transition-all duration-700 transform hover:scale-[1.02] hover:shadow-md",
                isInView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"
              )}
